...
class CheckoutStream implements StreamInterface {
    ...
    public function getContents() {
        ...
        if ($this->expectedLength !== null && floatval($length) !== $this->expectedLength) {
            $this -> throwOSSException($this->expectedLength, $length);
        }
        return $contents;
    }
        ...
        if ($this->expectedLength !== null) {
            ...
            if ($this->stream->eof()) {
                if (floatval($this->readedCount) !== $this->expectedLength) {
                    $this -> throwOSSException($this->expectedLength, $this->readedCount);
                }
            }
        }    
        return $string;
    ...
    public function throwOSSException($expectedLength, $reaLength) {
        $OSSException = new OSSException('premature end of Content-Length delimiter message body (expected:' . $expectedLength . '; received:' . $reaLength . ')');
        $OSSException->setExceptionType('server');
        throw $OSSException;
    }
}
